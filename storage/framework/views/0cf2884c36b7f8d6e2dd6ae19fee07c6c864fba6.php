<?php $__env->startSection('content'); ?>
    ...
    <div id="conteiner" class="container" data-date="<?= isset($dates)? $dates : '' ?>"
         ...
         data-singlestart = "<?=  isset($active['start']) ? $active['start'] : '' ?>" >

        <div class="row margin-top-twenty">
			<div class="span12">
            <div class="col-md-4 col-lg-4 btn-toolbar toolbar-span ">
                <div class="daterange daterange--single track-datepicker picker-width"></div>
            </div>
            <div class="col-md-4 col-lg-4 padding-twenty">
				<div class="row col-md-12 col-sm-12 col-xs-12 project-dropdown">
					<select name="users" class=" input-xlarge focused my_input input-height"   id="SelectAllUserTrackReport" data-all="true">
            ...
            <div class="col-md-4 col-lg-4 padding-twenty">
				<div class="row col-md-12 col-sm-12 col-xs-12 margin-ten report-text">
					<span class="pull-right font-thirty heading-color">Track Report</span>
				</div>
			</div>
			</div>
		...
		<div class="row col-md-12">
            <?php if((empty($active['start']) && empty($active['end']) && empty($active['userId'])) || !$trackReport->count()): ?>
			...
			<button id="update-column-button" class="btn btn-sm button-export update-column-button margin-left-twenty" data-name="track-report">
		...
		<!---  Mith 11/23/2017: uncomment below to show date-picker ranges to filter task according to two dates -->
    <!--
    <div class="row col-md-12">
                ...
                <div class="block-content collapse in">
                    <div class="span12">
                        <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered my_input" id="usersTable">
                            ...
                            <?php if(isset($trackReport)): ?>
								<?php $__currentLoopData = $trackReport; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); ?>
									<tr class="odd gradeX">
										...
										<td><?= isset($key->track_start) ? date("Y-m-d H:i:s", strtotime($key->track_start." UTC")) : ''; ?></td>
										<td><?= isset($key->finish_track) ? date("Y-m-d H:i:s", strtotime($key->finish_track." UTC")) : ''; ?></td>
									</tr>
								<?php endforeach; $__env->popLoop(); $loop = $__env->getFirstLoop(); ?>
    ...
    <script type="text/javascript">
       $(document).ready(function() {
         $('.button-excel').click(function() {
            ...
            tableHtml += "</table>";
			var fileName = "trackReport-"+todayDate();
            $('#trackexport').html(tableHtml);
            $('#trackexport').tableExport({type:'xlsx', excelstyles:['border-bottom', 'border-top', 'border-left', 'border-right'],fileName: fileName,worksheetName: 'trackReport'});
            $('#trackexport').html('');
         });
